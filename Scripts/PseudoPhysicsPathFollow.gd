...
func _on_ExitArea_body_entered(body):
	if looping_body == null:
		...
		if speed < 0:
			looping_body = weakref(body)
			looping_body.get_ref().delayed_teleport(exit_transform.origin)
			set_unit_offset(1.0)
...
func _physics_process(delta):
	...
	if looping_body.get_ref():
		...
		if reached_exit or reached_entrance:
			...
			if reached_entrance:
				looping_body.get_ref().apply_central_impulse(-entrance_transform.basis.z.normalized() * -speed / start_velocity_multiplier)
			reset(true)
		else:
...
func start_follow():
	...
	if looping_body.get_ref().get_collision_layer() == 1:
