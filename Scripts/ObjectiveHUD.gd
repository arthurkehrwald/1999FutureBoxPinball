...
func _on_MissionTracker_current_mission_changed(mission: Mission):
	if is_active and active_sub_state != mission_complete_state:
		.set_active_sub_state(new_mission_state)
	else:
...
func _ready():
	MissionTracker.connect("current_mission_changed", self, "_on_MissionTracker_current_mission_changed")
	MissionTracker.connect("current_mission_completed", self,"_on_MissionTracker_current_mission_completed")
